...
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://bot-1784567847-9672-voglogpro.bothost.tech/")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for city in _configured_cities():
            ...
            try:
                ...
                # не трогая ключ. Иначе запуск падал бы на UNIQUE(cities.group_id)
                # и веб-сервер вообще не поднимался.
                await db.execute(
                    "UPDATE cities SET name = ?, topic_tasks = ?, topic_npb = ?, "
                    "topic_reports = ?, timezone_offset = ?, is_active = 1, "
