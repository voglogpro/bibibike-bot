...
def _webapp_button():
    ...
    Если задан WEBAPP_URL — используем web_app-кнопку: приложение открывается
    в один тап поверх группы (без промежуточного чата с ботом).
    Иначе откатываемся на старую url-ссылку t.me/бот/shortname.
    """
    if WEBAPP_URL:
        return InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="⚡ Смена", web_app=WebAppInfo(url=WEBAPP_URL))]]
        )
    if BOT_USERNAME:
...
async def get_lifetime(uid):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        c = await db.execute("SELECT action_type, bike_codes, quantity FROM actions WHERE user_id = ?", (uid,))
        total = 0
...
async def api_action_add(request):
    ...
    # === Защита от минуса: при -1 нельзя опустить счётчик ниже нуля. ===
    if delta == -1:
        stats = await get_stats(shift["id"])
        if stats.get(atype, 0) <= 0:
            return web.json_response(
