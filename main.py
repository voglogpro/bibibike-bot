...
async def _process_work_message_locked(message: Message, city, npb=False, edited=False,
    ...
    # === ЭКСПЕРИМЕНТ: распознавание скриншотов техприложения ===
    # Работает ТОЛЬКО при включённом тумблере и ТОЛЬКО когда обычный
    # парсер ничего не нашёл в тексте. Если текст распознан — приоритет
    # у него, поведение бота не меняется вообще.
    if not actions:
        try:
            ...
            if author and author.get("photo_parse"):
                if getattr(message, "photo", None):
                    logger.info(
                        "ФОТО-СТАРТ: uid=%s msg=%s — тумблер включён, текста нет, "
            ...
            # условиях: при любой ошибке идём дальше с пустым результатом.
